...
func GetVideoList(ctx context.Context, req dto.QueryByQQRequest) (*dto.PageResponse[*model.Video], error) {
	if req.QQ == "" {
		return nil, common.ErrInvalidParam
	}
	page, pageSize := normalizePage(req.Page, req.PageSize)
	offset := (page - 1) * pageSize
	items, total, err := dao.ListVideos(ctx, req.QQ, offset, pageSize)
	if err != nil {
		return nil, err
	}
